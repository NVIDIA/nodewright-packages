...
install_from_aws() {
  ...
  echo "deb [signed-by=/etc/apt/keyrings/fsx-ubuntu-public-key.gpg] https://fsx-lustre-client-repo.s3.amazonaws.com/ubuntu jammy main" \
    | tee /etc/apt/sources.list.d/fsxlustreclientrepo.list > /dev/null && apt_with_dpkg_heal apt-get update
  ...
  if apt-cache show "lustre-client-modules-${FULL_KERNEL_VER}" >/dev/null 2>&1; then
    ...
    apt_with_dpkg_heal apt-get install -y "lustre-client-modules-${FULL_KERNEL_VER}"
    else
    ...
    if [ -z "${SKIP_SYSTEM_OPERATIONS:-}" ]; then
      apt_with_dpkg_heal apt-get install -y "${lustre_package}"
    else
