...
#
# Interactive release-tag helper. Prompts for a package and a bump
...
#
# Pushing a <package>/<version> tag triggers the CI release workflow, so the push
# is always a distinct, opt-in step. RC tags use the dotted `-rc.N` form so
...
# <package>/<base>-rc.<n>, or 1 if none exist.
next_rc() {
...
if [[ ${#PACKAGES[@]} -eq 0 ]]; then
    echo "${C_RED}ERROR: no packages found (no top-level dir contains config.json)${C_RESET}" >&2
    exit 1
fi
...
echo "${C_DIM}Reminder: the release commit (CHANGELOG cut, version bump) should already be committed at HEAD.${C_RESET}"
...
echo
echo "${C_BOLD}${C_RED}Pushing ${TAG} will TRIGGER THE CI RELEASE for this package.${C_RESET}"
