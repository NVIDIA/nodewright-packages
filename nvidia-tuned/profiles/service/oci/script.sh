...
#
# Re-enables IPv6 on mlx5 RDMA VFs that already exist at profile activation. The
# durable defaults for VFs created later live in the profile's [sysctl] section.
...
case "${cmd}" in
    start)
        ...
        ;;
    stop)
        # full_rollback (arg 2) - nothing to unapply here: the IPv6 defaults are unwound
        # by tuned's [sysctl] rollback, and the per-VF settings are transient and
        # disappear with the VFs themselves.
        ;;
    verify)
        verify_rdma_ipv6
        ;;
